...
function BlogCategory({ blog_tag }) {
  ...
  return (
    <div>
      <Head>
        <title>Cagir.az - Bloq {blog_tag}</title>
        <meta property="og:title" content={`Cagir.az - Bloq ${blog_tag}`} />
        <meta
          name="description"
          content={`Cagir.az - Bloq ${blog_tag} haqqında daha ətraflı və detallı məlumat əldə etmək üçün bu yazıya baş çəkib faydalana bilərsiniz. `}
        />
      </Head>
...
export async function getServerSideProps(context) {
  const blog_tag = context.params["blog_tag"];
  return {
    props: { blog_tag },
  };
}
